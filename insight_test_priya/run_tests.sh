...
function check_project_struct {
  find_file_or_dir_in_project ${PROJECT_PATH} main_run.sh
...
function setup_testing_input_output {
  ...
  cp -r ${PROJECT_PATH}/main_run.sh ${TEST_OUTPUT_PATH}
  ...
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/test_input.txt ${TEST_OUTPUT_PATH}/input/test_input.txt
}

function compare_outputs {
  NUM_OUTPUT_FILES_PASSED=0
  OUTPUT_FILENAME=sample_output_pharm.txt
